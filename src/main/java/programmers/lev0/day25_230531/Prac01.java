...
public class Prac01 {
    public static void main(String[] args) {
        ...
        int answer = prac01.solution2(new String[]{"p", "o", "s"}, new String[]{"sod", "eocd", "qixm", "adio", "soo"});
    ...
    public int solution(String[] spell, String[] dic) {
        ...
        for(int i=0; i<dic.length; i++) {               //dic의 길이만큼 for문을 돌림
            ...
            for(int j=0; j<spell.length; j++) {         //spell의 길이만큼 for문을 돌림
                if(dic[i].indexOf(spell[j])==-1) {      // !dic[i].contains(spell[j])   //만약 spell의 j번째 알파벳이 dic의 i번째 단어에 포함이 안되어있으면
                    check=-1;                           //check를 -1로
